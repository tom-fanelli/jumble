...
function initialize() {
    ...
    for (let i = 0; i < NUM_WORDS; i++) {
        ...
        answer.id = "answer" + i;
        answer.classList.add("slots");

        let br = document.createElement("br");

        document.getElementById("board").appendChild(word);
        document.getElementById("board").appendChild(answer);
        document.getElementById("board").appendChild(br);

        // make answer tiles
        for (let j = 0; j < i + FIRST_WORD_LENGTH; j++) {
            let tile = document.createElement("span");
            tile.id = i.toString() + "-" + j.toString();
            tile.classList.add("tile");
            tile.innerText = "";
            document.getElementById("answer" + i).appendChild(tile);
        }

        // set words
        var wordListX = "wordList" + (i + FIRST_WORD_LENGTH); 
        const daysSinceEpoch = Math.round(Date.now() / 8.64e7);
        var todaysWord = eval(wordListX + "[" + (daysSinceEpoch % wordListLength[i]) + "]").toUpperCase();
        gameWords.set(i, todaysWord);

        // jumble words
        var todaysShuffledWord = shuffleLetters(todaysWord);
        while (todaysShuffledWord === todaysWord) {
            todaysShuffledWord = shuffleLetters(todaysWord);
        }
        document.getElementById("word" + i).innerText = todaysShuffledWord;
    }
...
function generateKeyboard() {
    ...
    // set letters
    var letters = document.getElementById("word" + row).innerText;
...
function shuffleKeyboard() {
    ...
    var newShuffle = shuffleLetters(currLetters);
    while (newShuffle === currLetters || newShuffle === gameWords.get(row)) { // ensure shuffle is new
        newShuffle = shuffleLetters(currLetters);
    }
    ...
    document.getElementById("word" + row).innerText = newShuffle;
}
...
function processInput(e) {
    ...
    if ("KeyA" <= e.code && e.code <= "KeyZ") {
        ...
        if (0 < col && col <= row + FIRST_WORD_LENGTH) {
            col -=1;
            let currTile = document.getElementById(row.toString() + '-' + col.toString());
...
function update() {
    ...
    for (let i = 0; i < row + FIRST_WORD_LENGTH; i++) {
    ...
    // check guess
    if (guess == gameWords.get(row)) { // guess is correct
        recordTimes();
        document.getElementById("word" + row).innerText = guess.toUpperCase();
        for (let i = 0; i < col; i++) {
            let currTile = document.getElementById(row.toString() + '-' + i.toString());
    ...
    } else if (guess.length == gameWords.get(row).length) { // only animate if all tiles full
        for (let i = 0; i < col; i++) {
            let currTile = document.getElementById(row.toString() + '-' + i.toString());
...
function setShareableResults() {
    ...
    document.getElementById("results").innerText = "🏆 " + month + " " + day + ", " + year + " 🏆\n" + 
...
function copyToClipboard(elementId) {
    ...
    document.getElementById("cpy-alert").classList.remove("hide");
    document.getElementById("cpy-alert").classList.add("show");
    document.getElementById("cpy-alert-br").classList.remove("hide");
    document.getElementById("cpy-alert-br").classList.add("show");
}
